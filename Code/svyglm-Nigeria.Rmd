...
country_data <- subset(data1, country == "Nigeria")# Design Factors
...
# write.csv(summary_distribution, "Nigeria_distribution.csv", row.names = FALSE)
...
# save as a CSV
# for (var in names(prevalence_list)) {
#   write.csv(prevalence_list[[var]], paste0(var, "_Nigeria.csv"), row.names = FALSE)
# }
```
...
for (var in names(prevalence_list)) {
  ...
  variable_bar <- ggplot(df, aes_string(x = var, y = "experienced_fistula1", fill = var)) +
    ...
    geom_text(aes_string(label = "sprintf('%.2f%%', experienced_fistula1 * 100)"), 
              vjust = -0.5, size = 3) +  
    labs(x = var, y = "Prevalence of Fistula", title = paste("Prevalence of Fistula by", var, "in Nigeria")) +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
```{r}
# Final Model
glm_country_final <- svyglm(experienced_fistula ~ age + religion + wealth  
...
```{r}
# wealth*age
glm_country_wealth1 <- svyglm(experienced_fistula ~ age + religion + wealth  
                         + education + contraception_use + wealth*age,
                   design = svydesign_country, 
                   family = quasibinomial())
...
```{r}
# wealth*religion
glm_country_wealth2 <- svyglm(experienced_fistula ~ age + religion + wealth  
                         + education + contraception_use + wealth*religion,
                   design = svydesign_country, 
                   family = quasibinomial())
...
glm_country_wealth3 <- svyglm(experienced_fistula ~ age + religion + wealth  
                         + education + contraception_use + wealth*contraception_use,
                   design = svydesign_country, 
                   family = quasibinomial())
...
```{r}
# education*age
glm_country_education1 <- svyglm(experienced_fistula ~ age + religion + wealth  
                         + education + contraception_use + education*age,
                   design = svydesign_country, 
                   family = quasibinomial())
...
```{r}
# education*age
glm_country_education2 <- svyglm(experienced_fistula ~ age + religion + wealth  
                         + education + contraception_use + education*religion,
                   design = svydesign_country, 
                   family = quasibinomial())
...
```{r}
# education*contraception_use
glm_country_education3 <- svyglm(experienced_fistula ~ age + religion + wealth  
                         + education + contraception_use + education*contraception_use,
                   design = svydesign_country, 
                   family = quasibinomial())
