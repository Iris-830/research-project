...
country_data <- subset(data1, country == "SierraLeone")# Design Factors
...
# write.csv(summary_distribution, "SierraLeone_distribution.csv", row.names = FALSE)
...
# save as a CSV
# for (var in names(prevalence_list)) {
#   write.csv(prevalence_list[[var]], paste0(var, "_SierraLeone.csv"), row.names = FALSE)
# }
...
for (var in names(prevalence_list)) {
  ...
  variable_bar <- ggplot(df, aes_string(x = var, y = "experienced_fistula1", fill = var)) +
    ...
    geom_text(aes_string(label = "sprintf('%.2f%%', experienced_fistula1 * 100)"), 
              vjust = -0.5, size = 3) +  
    labs(x = var, y = "Prevalence of Fistula", title = paste("Prevalence of Fistula by", var, "in Sierra Leone")) +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
```{r}
# Final Model
glm_country_final <- svyglm(experienced_fistula ~ age + age_sex + number_birth + religion 
                            + wealth + occupation + contraception_use
...
```{r}
# wealth*age
glm_country_wealth1 <- svyglm(experienced_fistula ~ age + age_sex + number_birth + religion 
                            + wealth + occupation + contraception_use
                         + delivery_place + postpartum_care + wealth*age,
                   design = svydesign_country, 
                   family = quasibinomial())

summary(glm_country_wealth1)

# Calculate AIC
AIC(glm_country_wealth1)

# Calculate confidence intervals
conf_intervals <- confint(glm_country_wealth1)
print(conf_intervals)
```

```{r}
# wealth*number_birth
glm_country_wealth2 <- svyglm(experienced_fistula ~ age + age_sex + number_birth + religion 
                            + wealth + occupation + contraception_use
                         + delivery_place + postpartum_care + wealth*number_birth,
                   design = svydesign_country, 
                   family = quasibinomial())

summary(glm_country_wealth2)

# Calculate AIC
AIC(glm_country_wealth2)

# Calculate confidence intervals
conf_intervals <- confint(glm_country_wealth2)
print(conf_intervals)
```


```{r}
# wealth*contraception_use
glm_country_wealth3 <- svyglm(experienced_fistula ~ age + age_sex + number_birth + religion 
                            + wealth + occupation + contraception_use
                         + delivery_place + postpartum_care + wealth*contraception_use,
                   design = svydesign_country, 
                   family = quasibinomial())

summary(glm_country_wealth3)

# Calculate AIC
AIC(glm_country_wealth3)

# Calculate confidence intervals
conf_intervals <- confint(glm_country_wealth3)
print(conf_intervals)
```

```{r}
# wealth*delivery_place
glm_country_wealth4 <- svyglm(experienced_fistula ~ age + age_sex + number_birth + religion 
                            + wealth + occupation + contraception_use
                         + delivery_place + postpartum_care + wealth*delivery_place,
                   design = svydesign_country, 
                   family = quasibinomial())

summary(glm_country_wealth4)

# Calculate AIC
AIC(glm_country_wealth4)

# Calculate confidence intervals
conf_intervals <- confint(glm_country_wealth4)
print(conf_intervals)
```

```{r}
# wealth*postpartum_care
glm_country_wealth5 <- svyglm(experienced_fistula ~ age + age_sex + number_birth + religion 
                            + wealth + occupation + contraception_use
                         + delivery_place + postpartum_care + wealth*postpartum_care,
                   design = svydesign_country, 
                   family = quasibinomial())

summary(glm_country_wealth5)

# Calculate AIC
AIC(glm_country_wealth5)

# Calculate confidence intervals
conf_intervals <- confint(glm_country_wealth5)
print(conf_intervals)
```
...
```{r}
# occupation*age
glm_country_occupation1 <- svyglm(experienced_fistula ~ age + age_sex + number_birth + religion 
                            + wealth + occupation + contraception_use
                         + delivery_place + postpartum_care + occupation*age,
                   design = svydesign_country, 
                   family = quasibinomial())

summary(glm_country_occupation1)

# Calculate AIC
AIC(glm_country_occupation1)

# Calculate confidence intervals
conf_intervals <- confint(glm_country_occupation1)
print(conf_intervals)
```

```{r}
# occupation*age_sex
glm_country_occupation2 <- svyglm(experienced_fistula ~ age + age_sex + number_birth + religion 
                            + wealth + occupation + contraception_use
                         + delivery_place + postpartum_care + occupation*age_sex,
                   design = svydesign_country, 
                   family = quasibinomial())

summary(glm_country_occupation2)

# Calculate AIC
AIC(glm_country_occupation2)

# Calculate confidence intervals
conf_intervals <- confint(glm_country_occupation2)
print(conf_intervals)
```

```{r}
# occupation*number_birth
glm_country_occupation3 <- svyglm(experienced_fistula ~ age + age_sex + number_birth + religion 
                            + wealth + occupation + contraception_use
                         + delivery_place + postpartum_care + occupation*number_birth,
                   design = svydesign_country, 
                   family = quasibinomial())

summary(glm_country_occupation3)

# Calculate AIC
AIC(glm_country_occupation3)

# Calculate confidence intervals
conf_intervals <- confint(glm_country_occupation3)
print(conf_intervals)
```

```{r}
# occupation*contraception_use
glm_country_occupation4 <- svyglm(experienced_fistula ~ age + age_sex + number_birth + religion 
                            + wealth + occupation + contraception_use
                         + delivery_place + postpartum_care + occupation*contraception_use,
                   design = svydesign_country, 
                   family = quasibinomial())

summary(glm_country_occupation4)

# Calculate AIC
AIC(glm_country_occupation4)

# Calculate confidence intervals
conf_intervals <- confint(glm_country_occupation4)
print(conf_intervals)
```

```{r}
# occupation*delivery_place
glm_country_occupation5 <- svyglm(experienced_fistula ~ age + age_sex + number_birth + religion 
                            + wealth + occupation + contraception_use
                         + delivery_place + postpartum_care + occupation*delivery_place,
                   design = svydesign_country, 
                   family = quasibinomial())

summary(glm_country_occupation5)

# Calculate AIC
AIC(glm_country_occupation5)

# Calculate confidence intervals
conf_intervals <- confint(glm_country_occupation5)
print(conf_intervals)
```

```{r}
# occupation*postpartum_care
glm_country_occupation6 <- svyglm(experienced_fistula ~ age + age_sex + number_birth + religion 
                            + wealth + occupation + contraception_use
                         + delivery_place + postpartum_care + occupation*postpartum_care,
                   design = svydesign_country, 
                   family = quasibinomial())

summary(glm_country_occupation6)

# Calculate AIC
AIC(glm_country_occupation6)

# Calculate confidence intervals
conf_intervals <- confint(glm_country_occupation6)
print(conf_intervals)
```
