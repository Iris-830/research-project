...
# Filter data for Cote d'lvoire
country_data <- subset(data1, country == "Cote d'lvoire")# Design Factors
...
# write.csv(summary_distribution, "Cote_distribution.csv", row.names = FALSE)
```
...
# save as a CSV
#for (var in names(prevalence_list)) {
#  write.csv(prevalence_list[[var]], paste0(var, "_cote.csv"), row.names = FALSE)
#}
...
for (var in names(prevalence_list)) {
  ...
  variable_bar <- ggplot(df, aes_string(x = var, y = "experienced_fistula1", fill = var)) +
    ...
    geom_text(aes_string(label = "sprintf('%.2f%%', experienced_fistula1 * 100)"), 
              vjust = -0.5, size = 3) +  
    labs(x = var, y = "Prevalence of Fistula", title = paste("Prevalence of Fistula by", var, "in Cote d'Ivoire")) +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
```{r}
# Final Model
glm_country_final <- svyglm(experienced_fistula ~ age + religion + occupation 
                         + delivery_professional + postpartum_care,
                   design = svydesign_country, 
                   family = quasibinomial())
...
```{r}
# occupation*age
glm_country_occupation1 <- svyglm(experienced_fistula ~ age + religion + occupation 
                         + delivery_professional + postpartum_care + occupation*age,
                   design = svydesign_country, 
                   family = quasibinomial())
...
```{r}
# occupation*religion
glm_country_occupation2 <- svyglm(experienced_fistula ~ age + religion + occupation 
                         + delivery_professional + postpartum_care + occupation*religion,
                   design = svydesign_country, 
                   family = quasibinomial())
...
```{r}
# occupation*delivery_professional
glm_country_occupation3 <- svyglm(experienced_fistula ~ age + religion + occupation 
                         + delivery_professional + postpartum_care + occupation*delivery_professional,
                   design = svydesign_country, 
                   family = quasibinomial())
...
glm_country_occupation4 <- svyglm(experienced_fistula ~ age + religion + occupation 
                         + delivery_professional + postpartum_care + occupation*postpartum_care,
                   design = svydesign_country, 
                   family = quasibinomial())
